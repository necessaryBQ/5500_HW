...
test('GET /author/name', async () => {
  await supertest(app).get('/author/name')
    .expect(200)
    .then((res) => {
      expect(res.body.data).not.toEqual("John Doe");
    });
});

test('GET /author/pennkey', async () => {
  const name = await supertest(app).get('/author/name')
    .then(res => res.body.data);

  await supertest(app).get('/author/pennkey')
    .expect(200)
    .then((res) => {
      expect(res.body.data).not.toEqual("jdoe");
      expect(res.body.data).not.toEqual(name);
    });
});
...
test('GET /playlist/entrance_songs', async () => {
  await supertest(app).get('/playlist/entrance_songs')
    .expect(200)
    .then((res) => {
      expect(res.body.length).toStrictEqual(results.entrance_playlist_1.length)
    });
})
